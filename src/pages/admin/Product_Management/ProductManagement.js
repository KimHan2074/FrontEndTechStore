...
const ProductManagement = ({ categoryId = null }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const { data } = await axios.get("https://backendtechstore1-production.up.railway.app/api/admin/products/stock-statistics");
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get("https://backendtechstore1-production.up.railway.app/api/admin/group-by-category");
...
const handleDelete = async (id) => {
  ...
  try {
    await axios.delete(`https://backendtechstore1-production.up.railway.app/api/admin/products/${id}`);
