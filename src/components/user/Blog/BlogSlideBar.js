...
const BlogSlideBar = ({ selectedCategoryId, setSelectedCategoryId }) => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const latestRes = await axios.get("https://backendtechstore1-production.up.railway.app/api/user/blogs/status");
        setBlogPosts(latestRes.data);
        console.log("Latest blog data:", latestRes.data);
        const allBlogRes = await axios.get("https://backendtechstore1-production.up.railway.app/api/user/blogs/index");
    ...
    const fetchCategories = async () => {
      try {
        const categoryRes = await axios.get("https://backendtechstore1-production.up.railway.app/api/user/blogs/categories");
