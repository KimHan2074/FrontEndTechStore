...
const PaymentConfirmation = ({
  ...
  // ✅ Lấy trạng thái từ URL (sau khi redirect từ MoMo)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const momoStatus = params.get("status")
    if (momoStatus === "fail") {
      setStatus("Payment Failed ❌")
    } else if (momoStatus === "success") {
      ...
      <div className="main-card">
        <div className="card-content">
          ...
          <div className="section">
            <h3 className="section_title">Customer Information</h3>
            <div className="customer-info">
              <div className="info-item"><strong>FullName:</strong> {customerInfo.fullname}</div>
              <div className="info-item"><strong>Email:</strong> {customerInfo.email}</div>
              <div className="info-item"><strong>Phone:</strong> {customerInfo.phone}</div>
              <div className="info-item"><strong>Address:</strong> {customerInfo.address}</div>
            </div>
          </div>
          ...
          <div className="section">
            ...
            <div className="payment-method-row">
              <span className="payment-name">{paymentMethod}</span>
              <span className="processing-badge">{status}</span>
            </div>
          </div>
          ...
          <div className="price-summary">
            ...
            <div className="price-row total"><span>Total</span><span style={{ color: "#0070f3" }}>{formatter.format(total)}</span></div>
          </div>

          {/* Confirm Button */}
          <div className="button-container">
            <button onClick={onConfirmOrder} className="confirm-button">Confirm Order</button>
          </div>
        </div>
      </div>
    </div>
