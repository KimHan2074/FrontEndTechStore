...
const LatestHomePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/user/blogs/index");
    ...
    <div className="last-home-container">
      <div className="cards-container-last-home">
        {visibleCards.length === 0 ? (
          <p className="no-results-message-last-home">No matching articles found</p>
        ) : (
          visibleCards.map((card) => (
            <div
              key={card.id}
              className="card-last-home"
            >
              <img
