    ...
    const fetchAdminInfo = async () => {
      try {
        ...
        const userResponse = await axios.get(
          `https://backendtechstore1-production.up.railway.app/api/user/${fetchedUserId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          }
      ...
      <div>
        <div className="admin-profile">
          ...
          <div className="admin-profile-info">
            ...
            <p className="admin-email">{adminInfo.email || "admin@example.com"}</p>
          </div>
        </div>
              ...
              {item.name === "Product" && showProductSubmenu && (
                <div className="product-submenu-scroll">
                  {categories.map((cat) => (
                    <li
                      key={cat.id}
                      className={`admin-menu-item-active sub-item ${activeItem === `ProductCategory:${cat.id}` ? "active" : ""
                        }`}
                      style={{ paddingLeft: 40 }}
                      onClick={() => setActiveItem(`ProductCategory:${cat.id}`)}
                    >
